...
```bash
scripts/package_macos.sh 0.6.0            # 用主機架構
scripts/package_macos.sh 0.6.0 arm64      # DMG 檔名帶架構後綴
...
# 產出 dist\macpad++-0.6.0-x64.zip（windeployqt 同梱 Qt/WebEngine/QScintilla，於乾淨機器可執行）
```
...
單元測試以 QtTest 撰寫，連結核心邏輯（NFR-008）；共 **47 套件**。
...
**v1 核心**：多分頁（拖曳/關閉/未存確認/**標色/唯讀鎖定**）、**分割視窗**、語法高亮、
...
**v0.6.0（對齊 Notepad++ v8.9.7）**：128 種語言、45 種語言的 Function List 規則、釘選分頁與多列
分頁列、Windows… 文件管理對話框、RTF 匯出、FormFeed 分頁列印、更新檢查。完整對照見
