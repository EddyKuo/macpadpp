...
## 📌 後續補記（2026-07-30，v0.6.0）

上表的 **tabBarMultiLine** 與 **autoUpdater** 兩項此後皆已實作，因此下方歷史章節
「僅剩兩項平台本質限制」的說法已非現況：

- **tabBarMultiLine** —— 自寫 `ui/MultiRowTabBar`（`QTabBar` 子類別）接管繪製與命中測試，
  真正跨列換行，取代先前的捲動按鈕近似。
...
> ⚠️ 已被取代 —— 見上方 2026-07-30 補記，兩項此後皆已實作。

除上述收斂外，本稽核清單中其餘可實作的 partial/missing 項目（CLI 旗標、搜尋選項記憶、Column Editor
repeat/text 模式、Split View 旋轉、codepoint 範圍搜尋等）亦已在 Sprint 6/7/7.1 陸續補齊。**當時唯二仍未實作
...
> 附註：其中「檔案唯讀屬性」與「系統匣」兩項後來確認為誤判並已實作，見本文最上方的雙平台重新認定。
