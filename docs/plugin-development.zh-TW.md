...
[English](plugin-development.md) · **繁體中文**
...
void InsertDividerExtension::onLoad(IHostServices *host)
{
    ...
    m_host->addMenuAction(QStringLiteral("Edit"), QStringLiteral("Insert Divider"), [this] {
        ...
        if (!editor) {
            m_host->showStatusMessage(tr("沒有作用中的編輯器"));
            return;
        }
        editor->insert(QStringLiteral("// ────────────────────────────\n"));
        m_host->showStatusMessage(tr("已插入分隔線"), 2000);
    });
}
...
- [ ] 用到 `.qrc` 的話，`onLoad` 開頭有呼叫 `Q_INIT_RESOURCE`（見 §8）
