...
[English](windows_prd.md) · **繁體中文**

> **來源**：[`docs/plan.zh-TW.md`](plan.zh-TW.md)（Windows 10/11 移植計畫 v1）
> **產品定位**：讓現有 macOS 版 macpad++（Notepad++ 對等原生編輯器）以**同一份原始碼**在
> Windows 10 (1809+) / Windows 11 上原生執行、建置、打包與發佈。
> **不改變**：既有 macOS 版功能、UI 行為、契約系統、品質門檻。

> **狀態附註（2026-07-30）**：本文件所描述的移植已完成並合併，Windows 版隨每次發佈一併產出。
> 本文件保留為歷史產品契約。
...
| # | 目標 | 衡量 |
|---|------|------|
| G-1 | Windows 使用者可安裝並執行 macpad++，功能與 macOS 版對等 | 對照 [`docs/parity.zh-TW.md`](parity.zh-TW.md) 逐項通過 |
...
| Windows 開發者 | clone → 依 `BUILD.zh-TW.md` Windows 章節，用 Visual Studio/MSVC 建置 |
...
- Given 移植完成
- When 使用者查閱文件
- Then `BUILD.zh-TW.md` 有 Windows 建置章節、`README.zh-TW.md` 有 Windows 安裝說明、設定路徑說明更新為 `%APPDATA%`
...
---
*本 PRD 為 Windows 移植之產品層契約；下游 SRS（[`windows_srs.zh-TW.md`](windows_srs.zh-TW.md)）、
SA/SD（[`windows_sa_sd.zh-TW.md`](windows_sa_sd.zh-TW.md)）、
詳細設計（[`windows_design.zh-TW.md`](windows_design.zh-TW.md)）依此展開。*
