...
> **狀態附註（2026-07-30，v0.6.0）**：下方 §1 把本專案描述為僅支援 macOS，那是撰寫當時的狀態。
> 自 v0.5.0 起 macpad++ 以單一份原始碼同時發行 **macOS 與 Windows 10/11**；移植架構見
> [`windows_sa_sd.zh-TW.md`](windows_sa_sd.zh-TW.md)，v0.6.0 的變更見 §20。
...
> ⚠️ 已被 §20 取代：上述兩項此後皆已於 v0.6.0 實作。
...
## 20. v0.6.0 — 對齊 Notepad++ v8.9.7

> 本輪把對照基準由「v8.7 之前」改為上游 v8.9.7（2026-03），補上這段期間新增的功能，
> 並重新檢視 §18.3 稱為「平台本質限制」的兩項。完整對照見 [`parity.zh-TW.md`](parity.zh-TW.md)。

### 20.1 新增模組
| 子系統 | 新檔案 | 說明 |
|--------|--------|------|
| 128 種語言 | `features/langs/BuiltinLanguages` | 95 種語言的資料表，按需轉為 `UdlDefinition` —— **把既有 UDL 引擎當成通用資料驅動 lexer**，因此不需要寫任何新 lexer 程式碼。`LexerFactory` 在原生 lexer 之後退回到它。 |
| 多列分頁列 | `ui/MultiRowTabBar` | `QTabBar` 子類別，接管繪製與命中測試，把分頁真正排成多列。`MultiRowTabWidget` 存在的唯一理由是 `QTabWidget::setTabBar` 為 protected。 |
...
- **重用 UDL 引擎，而非寫 95 個 lexer**：把每種語言表達為資料（關鍵字、註解符號、運算子、折疊標記）
  餵給既有 `UdlLexer`，語言覆蓋率因此由 33 提升到 128。代價是這些語言得到的是 UDL 等級而非
  文法等級的高亮；效益是新增一種語言只是一列表格，不是一個類別。
- **接管 `QTabBar` 繪製，而非接受工具組限制**：§18.3 把 `tabBarMultiLine` 記為做不到，那是錯的——
...
- **應用層的選取歷史**：隨附的 QScintilla 2.14.1 綁 Scintilla 5.3，沒有上游用的
  `SCI_SETUNDOSELECTIONHISTORY`（Scintilla 5.4）。改以 `EditorWidget` 內的選取快照堆疊
  （`undoWithHistory`/`redoWithHistory`）達成相同的使用者可見行為。
...
`QsciPrinter`，完全忽略所有列印偏好（已與工具列路徑統一）；唯讀鎖頭前綴會被 `updateTabTitle()` 覆寫。
另外發現 Folder as Workspace 的根目錄從未持久化，現已補上。
