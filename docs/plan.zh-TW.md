...
> **狀態**：草案 v1 —— **已於 2026-07-29 完成並合併**，本文保留為歷史計畫。
...
- [x] 安裝 **Qt 6.5+ for MSVC 2019/2022**（含 `WebEngineWidgets`、`Core5Compat`、`Svg`）。建議用官方 Qt Online Installer 或 aqtinstall。
- [x] 取得 **QScintilla for Qt6**：
  - 首選：**vcpkg**（`vcpkg install qscintilla`），或
  - 從 Riverbank 原始碼以 `qmake` 建置 `qscintilla2_qt6.dll`，或
  - 使用 PyQt 發佈的預編譯庫。
- [x] 安裝 **CMake ≥ 3.21**、**Ninja**、**MSVC 工具鏈**（Visual Studio 2022 Build Tools）。
- 驗收：`cmake --version`、`qmake --version`、找得到 `Qsci/qsciscintilla.h`。

### Phase 1 — 建置系統跨平台化（項目 A、B）
- [x] **`CMakeLists.txt`**：QScintilla 探測加入 Windows 分支。
...
- [x] **`src/CMakeLists.txt`**：可執行檔目標平台化。
...
- [x] **警告旗標**：`STRICT_WARNINGS` 目前寫死 `-Wall -Wextra -Werror`（GCC/Clang 語法）。MSVC 需改為 `/W4 /WX`。用 `if(MSVC) ... else() ... endif()` 分流。**（重要：否則 Windows 直接編不過）**
- [x] 準備 Windows 圖示：由現有 `.icns`/`.svg` 產生 `resources/icon/macpad.ico`（多尺寸 16–256px）。
- 驗收：`cmake -S . -B build -G Ninja` 設定成功、`cmake --build build` 連結出 `macpad++.exe`。

### Phase 2 — 執行期行為跨平台化（項目 C–H）
- [x] 新增 `src/platform/DesktopIntegration.{h,cpp}`（見 §2），實作各函式的 `Q_OS_WIN` / `Q_OS_MACOS` 分支：
...
- [x] 改寫呼叫點 C–G 改呼叫上述 API，移除硬編 `open`。
- [x] **`EditorWidget.cpp`**（3 處）：`QFont(QStringLiteral("Menlo"), 13)` 改為 `QFont(platform::defaultMonospaceFamily(), 13)`。保留既有 `setStyleHint(QFont::Monospace)` 作最終後備。
- 驗收：Reveal / Terminal / Browser / 字型在 Windows 實機皆正常。

### Phase 3 — 打包與發佈（項目 I、J）
- [x] 新增 **`scripts/package_windows.ps1`**：
...
- [x] **`.github/workflows/ci.yml`**：新增 `windows-latest` job（安裝 Qt via `jurplel/install-qt-action`、QScintilla via vcpkg、以 `cl` 建置並跑 CTest）。
- [x] **`.github/workflows/release.yml`**：新增 Windows build job，產出 `.exe`/`.msi` 上傳為 release artifact。
- 驗收：乾淨 Windows 10 與 11 各一台安裝後可啟動、無缺 DLL。

### Phase 4 — 測試與驗收
- [x] `ctest` 全綠（測試本身用 `/tmp/...` 常量字串僅為 in-memory 序列化資料，**非實際檔案路徑**，Windows 不受影響——已確認）。
- [x] 手動 parity 測試：對照 [`docs/parity.zh-TW.md`](parity.zh-TW.md) 逐項驗證，重點在 Windows 專屬互動：
  - Ctrl+雙擊選整字（原 ⌘）、Ctrl+Click 多游標、Alt+拖曳欄選。
  - 檔案關聯 / 拖放開檔、命令列多檔開啟、`-settingsDir` 等旗標。
  - Reveal in Explorer、Run 面板、Markdown 預覽（WebEngine）。
- [x] DPI 縮放：Windows 高 DPI（150%/200%）下 UI 與 QScintilla 邊界正常。
- [x] 更新文件：`BUILD.zh-TW.md`（加 Windows 章節）、`README.zh-TW.md`、`docs/design.zh-TW.md`（設定路徑 `%APPDATA%`）。
...
---
*本計畫原為草案；核心程式碼無架構性阻礙，主要成本在建置/打包基礎設施。
移植已於 2026-07-29 完成並併入 `main`（v0.5.2）。*
