...
[English](windows_design.md) · **繁體中文**

> **上游**：[PRD](windows_prd.zh-TW.md) / [SRS](windows_srs.zh-TW.md) / [SA-SD](windows_sa_sd.zh-TW.md)。
> 本文為實作藍圖，逐檔說明「改哪裡、改成什麼」。實作以此為準；完成後對照 §12 驗收清單。
...
- [x] `cmake -S . -B build` 在 Windows 設定成功（找到 Qt + QScintilla）
- [x] `cmake --build build` 連結出 `macpad++.exe`（無警告，`/WX` 下）
- [x] `ctest --output-on-failure` 全綠（含新測試）
- [x] 無任何原始檔內嵌 `open ` 平台指令（已全收斂至 DesktopIntegration）
- [x] `EditorWidget` 不再硬編 `Menlo`
- [x] `package_windows.ps1` 產出可執行封裝
- [x] CI/Release 具 Windows job
- [x] `BUILD.md`/`README.md` 更新
- [x] macOS 分支邏輯未被破壞（條件式保留原行為）
