...
[![Coverage](https://img.shields.io/badge/coverage-90%25-brightgreen)](docs/design.zh-TW.md)

**與 Notepad++ 對等的原生跨平台（macOS / Windows）文字／程式碼編輯器。** 以 C++17 + Qt6 + QScintilla
...
> Notepad++ 的原生 `.dll` 外掛 ABI、登錄檔關聯等**確屬平台綁定**的項目，改以自建的 in-process
> extension protocol 取代；其餘可實作的功能皆已補完，且**每一個新增偏好都有真實 runtime 效果
> （無死設定）**。自 v0.6.0 起，功能對齊 Notepad++ **v8.9.7**。完整對照、以及僅剩四項排除項目與其
> 理由，見 **[`docs/parity.zh-TW.md`](docs/parity.zh-TW.md)**。

---

## 特色一覽

- **多分頁編輯** — 拖曳排序、關閉確認、分頁標色、唯讀鎖定、**釘選分頁**與 Close All BUT Pinned、
  **多列分頁列**，以及完整的**分頁右鍵選單**（Close / Close All but This / Close to Left·Right、
  Save/Rename、Reload、Open Containing Folder、Copy 路徑…）。**雙 View 分割視窗**（水平/垂直、
  可旋轉方向、Move/Clone to Other View）含同步捲動與可選的同步縮放。
...
- **語法高亮 — 128 種語言** — 皆可手動指定，Language 選單依首字母分群，與上游做法一致；另支援
  **自訂語言（UDL）** 圖形化建立、Prefix Mode，以及與 Notepad++ 相容的 `userDefineLang.xml` 匯入/匯出。
- **Function List 支援 45 種語言** — 內建解析規則對齊上游 `functionList.xml`，並可由外部設定檔調整。
- **多游標 / 欄位編輯** — ⌘/Ctrl+Click 多游標、⌥/Alt+拖曳矩形選取、欄位插入遞增數列（重複次數 +
  Text 模式、可選進位基數）、Column→Multi-Edit 一鍵轉換。
- **強大搜尋** — 尋找/取代（正則、Extended `\u\b\o\d`）、**Find in Files**（背景可取消）、
  **Project Panel + Find in Projects**（多根專案樹狀管理、對專案內檔案清單搜尋）、增量搜尋含
  「第 n / 共 m 筆」計數、標記全部、書籤進階操作。
...
- **檢視** — 折疊（至第 N 層，可選 Simple/Circle/Box/Arrow 折疊標記樣式）、Document Map /
  **Function List** / Document List 面板（含 hover 預覽）、**Folder as Workspace**（根目錄與展開狀態
  跨 session 保留）、Monitoring（tail -f）、全螢幕、Distraction Free、Post-It、瀏覽器預覽。
- **自動化** — 巨集錄製/播放/具名儲存 + **管理對話框**（Modify Shortcut/Delete/Rename）、外部命令執行
  （可存具名命令並**各自綁定快捷鍵**）、Style Configurator（含主題下拉套用）、
  Shortcut Mapper（依來源分頁 + 衝突偵測）。
- **內建 17 套主題** — 複刻大廠 IDE（Monokai、Dracula、One Dark、Nord、Solarized 深/淺、
...
- **原生系統整合（macOS / Windows）** — 跟隨系統深/淺色、單一實例（可設定 multi-instance 模式）、
  ...
  （32 個按鈕、8 條分隔線，順序完全比照上游 `toolBarIcons[]`，並可逐顆隱藏）。造型採
  [Phosphor Icons](https://phosphoricons.com/)（MIT），隨主題自動變色——Notepad++ 自身的圖示為 GPLv3，
  無法在 MIT 專案中沿用。
- **完整 Preferences** — 涵蓋 New Document / Editing / Print / Tab Bar / Toolbar / Margins·Border·Edge /
  ...
  色彩模式、邊界，以及 FormFeed 視為分頁符。
- **匯出** — HTML 與 RTF 匯出，皆保留語法色彩。
...
> 「自行建置」從原始碼編譯（見 **[BUILD.zh-TW.md ▸ Windows 建置](BUILD.zh-TW.md#windows-建置)**）。
...
- **Windows** 10/11 ＋ Visual Studio 2022（MSVC）＋ CMake ＋ Qt6 ＋ QScintilla
  （見 [BUILD.zh-TW.md ▸ Windows 建置](BUILD.zh-TW.md#windows-建置)）
...
> Windows 完整步驟（取得 Qt、建置 QScintilla、打包）見
> **[BUILD.zh-TW.md ▸ Windows 建置](BUILD.zh-TW.md#windows-建置)**。
...
更多細節見 **[`BUILD.zh-TW.md`](BUILD.zh-TW.md)**。
...
**[`docs/design.zh-TW.md`](docs/design.zh-TW.md)**；功能對照見
**[`docs/parity.zh-TW.md`](docs/parity.zh-TW.md)**；所有文件（雙語）索引見
**[`docs/README.zh-TW.md`](docs/README.zh-TW.md)**。
...
想自己寫外掛？完整教學（含如何掛載進來）見
**[`docs/plugin-development.zh-TW.md`](docs/plugin-development.zh-TW.md)**。
...
```bash
# 1. 更新 CMakeLists.txt 的 project(... VERSION x.y.z)
# 2. 打 annotated tag（tag 說明會成為 Release notes）並 push
...
```bash
# macOS
scripts/package_macos.sh 0.6.0            # 產出 dist/macpad++-0.6.0-<arch>.dmg
```
```powershell
# Windows（於 VS Native Tools 命令列，Qt bin 在 PATH）
pwsh scripts/package_windows.ps1 -Version 0.6.0   # 產出 dist\macpad++-0.6.0-x64.zip
```
...
第三方資源授權聲明見 **[THIRD-PARTY-NOTICES.zh-TW.md](THIRD-PARTY-NOTICES.zh-TW.md)**。
